    ...
    if valueToExpect != expectedValue:
        if not useWarnings:
            ...
            bpy.context.window_manager.popup_menu(
                lambda self, context: self.layout.label(text=errorString), title="Error", icon='ERROR')
...
def loadTexture(mesh, textureType, material, file):
    ...
    if name is not None:
        ...
        if offset is not None:
            slot.offset[0] = offset.values[0]
            slot.offset[1] = offset.values[1]
        scale = BMDLShaderParamFloat.getParameter(mesh["shaderFloatParams"], "TileUV")
        if scale is not None:
            slot.scale[0] = scale.values[0]
            slot.scale[1] = scale.values[1]
...
def importBMDL(file):
    ...
    try:
        ...
        for mesh in meshes:
            bounds = []
            for i in range(0, 8):
                bounds.append(struct.unpack('f', file.read(4))[0])
            mesh["bounds"] = bounds
            mesh["unkInt"] = struct.unpack('i', file.read(4))[0]  # ?
            mesh["firstIndex"] = struct.unpack('i', file.read(4))[0]
            mesh["indicesCount"] = struct.unpack('i', file.read(4))[0]
...
class BMDLSectionBounds():
    ...
    def __repr__(self):
        return f"BMDLSectionBounds [dataOffset={self.dataOffset}, bounds={str(self.bounds)}]"
...
class BMDLSectionObject:
    def __init__(self, file, base_offset=0):
        # Offset to string
        self.name_offset = readInt(file)
        self.hash = read_uint(file)
        self.unk = readInt(file)
        self.count = readInt(file)
        file.seek(base_offset + self.name_offset)
        self.name = read_string(file)

    def __str__(self):
        return "BMDLSectionObject [name_offset=%d, hash=%x, unk=%d, count=%d, name=%s]" % \
               (self.name_offset, self.hash, self.unk, self.count, self.name)


class BMDLSectionBounds2:
    def __init__(self, file):
        self.data_offset = readInt(file)
        self.bounds = []
        # 8 floats ?
        for _ in range(0, 8):
            self.bounds.append(read_float(file))

    def __str__(self):
        return "BMDLSectionBounds2 [data_offset=%d, bounds=%s]" % (self.data_offset, str(self.bounds))
...
class BMDLSectionOffset:
    ...
    def __str__(self):
        return "BMDLSectionOffset [data_offset=%d]" % self.data_offset
...
class BMDLVertexFormat:
    ...
    def __init__(self, file):
        ...
        while True:
            num = struct.unpack("<H", file.read(2))[0]
            if num == 0x00FF:
                break
            file.seek(2, 1) # skip offset
            file.seek(2, 1) # skip unk
            method_id = struct.unpack("<H", file.read(2))[0]
            self.fmt.append(self.methods[method_id])
